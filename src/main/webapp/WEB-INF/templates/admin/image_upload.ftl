...
function list_img(start, size){
	...
	$.getJSON("list_image.do?start=" + start + "&size=" + size + "&category=" + slt_img_category + "&nocache=" + new Date(),function(data) {
		if(data!=null && data != "" && data.state == "SUCCESS"){
			total = data.total;
...
function initImgSlt(inpEleId, initContent, isMulti, isTdShow, changeCall){
	...
	if(isMulti){
		...
		if(initContent){
			$("#single_img_show_img_" + inpEleId).prop("src", initContent);
			$("#single_img_show_div_" + inpEleId).css("display","block");
			$("#"+inpEleId)[0].value = initContent;
		}
	}
	if(typeof changeCall != undefined){
...
function addShowHtml(inpEleId, isMulti, isTdShow){
	if(isMulti){
		$("#" + inpEleId).after("<div class='col-sm-9' id='multi_img_show_div_" + inpEleId + "' style='padding:0px'><div id='multi_img_show_items_" + inpEleId + "' class='col-sm-12' style='padding:0px'></div>" +
		...
		if(isTdShow){
			$("#" + inpEleId).after("<div style='padding:0px'><div id='single_img_show_div_" + inpEleId + "' style='display:none;padding:0px'><div style='padding:0px;border:2px solid #f2f2f2;background-color:#f2f2f2;margin-bottom:5px;display:inline-block;overflow:hidden;height:75px;width:94px;'><img id='single_img_show_img_" + inpEleId + "' src='' width='100px'/>" +
			...
			$("#" + inpEleId).after("<div class=''><div class='col-sm-12' id='single_img_show_div_" + inpEleId + "' style='display:none;padding:0px'><div style='padding:0px;border:3px solid #f2f2f2;background-color:#f2f2f2;margin-top:10px;margin-bottom:10px;float:left;overflow:hidden;height:100px;width:100px;'><img id='single_img_show_img_" + inpEleId + "' src='' width='100px'/>" +
...
function clearSingleSlt(eleId){
	$("#" + eleId).val("");
	if(typeof changeCallFunc != "undefined" && changeCallFunc != null){
		changeCallFunc($("#" + eleId), "");
	}
	$("#single_img_show_img_" + eleId).prop("src", "");
...
function sltImgDone(){
	if(isSingleMode){
		if(currentSltUrl){
			$("#single_img_show_img_" + currentDom.id).prop("src", currentSltUrl);
...
function addImgToList(elementId, url, isDeletable, inpName){
	...
	var div = $("<div class='select_list_img_div' title='" + url + "' isSlt='' id='show_div_" + count + "' style='position:relative;border:3px solid #f2f2f2;background-color:#f2f2f2;margin-right:30px;margin-top:10px;margin-bottom:10px;float:left;overflow:hidden;height:100px;width:100px'>");
